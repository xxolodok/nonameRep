...
void hashtable_init(HashTable* table, size_t capacity, LinearAllocator* allocator) {
    if (!table || !allocator || capacity == 0) return;
...
static int find_slot(HashTable* table, const char* key, size_t *out_index, int for_insert) {
    ...
    while (1) {
        ...
        if (index == start_index) break; // Обошли всю таблицу
    }
...
void hashtable_insert(HashTable* table, const char* key, void* value) {
    if (!table || !key || table->capacity == 0) return;
    ...
    if (item->is_occupied) {item->value = value; return;}
...
void* hashtable_get(HashTable* table, const char* key) {
    if (!table || !key || table->capacity == 0) return NULL;
    ...
    HashTableItem *item = &table->items[index];
    if (item->is_occupied) return item->value;
